...
public class ZenworksBuildCopy {
	...
	private SSHTerminalConsole dockerContainerTerminal;
    ...
    public void login(String userName, String password, String aditionalCriteria) throws TerminalLoginException {
		...
		dockerContainerTerminal = new SSHDockerContainerTerminalConsole(containerName,new PipedOutputStream(),new PipedInputStream());
		dockerContainerTerminal.login(userName, password, aditionalCriteria);
	...
	public String copyjar(String fileName) throws TerminalExecutionException {
		...
		try {
			...
			String tempLocation = "/tmp/" + getTrimJarVersion(file.getName());
			copyFileTerminal.executeCommand("src=" + fileName + " out=" + tempLocation);
            createBackup(jarName);
            System.out.println("Copying to container");
            copyToContainer(tempLocation,jarName);
			return "Done!!!";
		} catch (Exception e) {
			e.printStackTrace();
		}
    ...
    private void createBackup(String jarName) throws TerminalExecutionException {
		...
		{
			i++;
			findjar= dockerContainerTerminal.executeCommand("dir " + file.getParent().replaceAll("\\\\", "/")  + " | grep '" + file.getName() +i+ ".backup'");
		
		}while(findjar != null &&! findjar.isEmpty());*/
		
		
		findjar= dockerContainerTerminal.executeCommand("dir " + file.getParent().replaceAll("\\\\", "/")  + " | grep '" +file.getName().replace(getExtension(file.getName()), "")+ ".backup'");
		if(findjar == null || findjar.isEmpty())
		{
			dockerContainerTerminal.executeCommand("mv '" + jarName + "' '"+jarName.replace(getExtension(file.getName()), "")+ ".backup'");
		}
	...
	private String getJarLocation(String jarName) throws JSchException, IOException, TerminalExecutionException {
		...
		jarName = getTrimJarVersion(jarName);
	...
	private String getTrimJarVersion(String jarName)
	{
	...
	private String getLibLocation(String jarName) throws JSchException, IOException, TerminalExecutionException {
		String findjar = dockerContainerTerminal.executeCommand("find " + JAVA_JAR_LIB + " -name '" + jarName + "'");
		if (findjar != null && !findjar.isEmpty()) {
			return JAVA_JAR_LIB;
		}
		findjar = dockerContainerTerminal.executeCommand("find " + ZENWORK_JAR_LIB + " -name '" + jarName + "'");
		if (findjar != null && !findjar.isEmpty()) {
			return ZENWORK_JAR_LIB;
		}
		
		findjar = dockerContainerTerminal.executeCommand("find " + ZENWORK_WEB_LIB + " -path  \\*/" + jarName + "");
		if (findjar != null && !findjar.isEmpty()) {
...
//find /opt/novell/zenworks/share/tomcat/webapps/ -name
	...
	private void copyJars(List<String> jarList) throws TerminalExecutionException {
        ...
        containerId = sshTerminal.executeCommand("docker ps -a -q -f  name=" + containerName).trim();
	...
	private void disconnect() throws TerminalException {
		...
		dockerContainerTerminal.disconnect();
